...
class Log
{
	...
	public $Log_Status = Log::LOG_CLOSED;
	public $DateFormat = "Y-m-d G:i:s";
	public $MessageQueue;

	private $filename;
	private $log_file;
	private $priority = Log::INFO;
	...
	public function __construct( $filepath, $timezone, $priority )
	{
		...
		if ( !file_exists($filepath) )	//判断文件路径是否存在
		{
			if(!empty($filepath))	//判断路径是否为空
			{
				...
				{
					$this->Log_Status = Log::OPEN_FAILED;
	...
	public function LogInfo($line)
	{
		...
		$this->Log( $line, Log::INFO, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	public function LogDebug($line)
	{
		...
		$sGetFilePath = $sAarray[0]["file"];
		$sGetFileLine = $sAarray[0]["line"];
		$this->Log( $line, Log::DEBUG, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	public function LogWarn($line)
	{
		...
		$sGetFilePath = $sAarray[0]["file"];
		$sGetFileLine = $sAarray[0]["line"];
		$this->Log( $line, Log::WARN, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	public function LogError($line)
	{
		...
		$sGetFilePath = $sAarray[0]["file"];
		$sGetFileLine = $sAarray[0]["line"];
		$this->Log( $line, Log::ERROR, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	public function LogFatal($line)
	{
		...
		$sGetFilePath = $sAarray[0]["file"];
		$sGetFileLine = $sAarray[0]["line"];
		$this->Log( $line, Log::FATAL, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	{
		if ( $this->Log_Status == Log::LOG_OPEN && $this->priority != Log::OFF )
		{
